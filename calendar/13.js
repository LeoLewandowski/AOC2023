...
function showMountain(m, mirror) {
    ...
    m.forEach((l,y) => {
        ...
        for(let x = 0; x < l.length; x++){
            if(mirror[2] == 1 && (x == mirror[1] || x == mirror[1] - 1)) s += '\x1b[31m' + l[x] + '\x1b[0m';
            else if(mirror[2] == 1 && x >= mirror[1] - mirror[0] && x < mirror[1] + mirror[0]) s += '\x1b[32m' + l[x] + '\x1b[0m';
            else s += l[x];
        }
        if(mirror[2] == 100 && (y == mirror[1] || y == mirror[1] - 1)) s = '\x1b[31m' + s + '\x1b[0m';
        else if(mirror[2] == 100 && y >= mirror[1] - mirror[0] && y < mirror[1] + mirror[0]) s = '\x1b[32m' + s + '\x1b[0m';
        o += s + '\n';
    });
    console.log(o);
}
...
function findMirror(mountain, part = 0) {
    ...
    for (let y = 1; y < mountain.length; y++) {
        ...
        if (ref[0]) {
            ...
            if(y - s == 0 || mountain.length - y - s == 0) mirror = [s,y,100];
        }
    }
    for (let x = 1; x < mountain[0].length; x++) {
        let ref = compareReflections(getColumn(mountain,x - 1), getColumn(mountain,x), part);
        if(ref[0]){
            let s = getReflectionSize(x, mountain, false, part);
            if(x - s == 0 || mountain[0].length - x - s == 0) mirror = [s,x,1];
        }
    }
    return mirror;
}
...
function getReflectionSize(mirrorPos, mountain, horizontal, part){
    let size = 0, maxLen = (horizontal ? mirrorPos : mirrorPos), jokers = part;
    for(let i = 0; i < maxLen; i++){
        let line1 = (horizontal ? getRow(mountain,mirrorPos - i - 1) : getColumn(mountain,mirrorPos - i - 1));
        let line2 = (horizontal ? getRow(mountain,mirrorPos + i) : getColumn(mountain,mirrorPos + i));
